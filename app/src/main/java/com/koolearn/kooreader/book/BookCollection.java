...
public class BookCollection extends AbstractBookCollection<DbBook> {
    ...
    private final BooksDatabase myDatabase; //数据库
    public final List<String> BookDirectories; //目录
     ...
     *
     * @param book  要删除的book
     * @param deleteFromDisk   是否从sdcard删除
     */
    ...
    private void build() {
        ...
        final Set<DbBook> orphanedBooks = new HashSet<>();
        final Set<ZLPhysicalFile> physicalFiles = new HashSet<>();
        int count = 0;
        for (DbBook book : savedBooksByFileId.values()) {
            final ZLPhysicalFile file = book.File.getPhysicalFile();
            if (file != null) {
                physicalFiles.add(file); //添加这个文件
                //我们把这个文件保存到物理路径下(如果不在进行稍后的阅读)
                // yes, we do add the file to physicalFiles set
                //      for not testing it again later,
                // but we do not store the book 我们不保存这个book
        ...
        final Set<DbBook> newBooks = new HashSet<>();
        ...
        // Step 4: save changes into database  //保存改变的文件到数据库
        fileInfos.save();
